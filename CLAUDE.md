...
| `products.deleted_at` (timestamptz, 0006) | 软删除时间戳，NULL=live，非空=回收站（public/AI 过滤掉，可恢复） | super_admin / editor 自己产品 |
...
- [x] **接 rate limiter** —— ✅ 已挂（2026-05 F1）。`server/src/app.ts`
      上 `/api/generate` 限 10/min、`/api/stt` 限 20/min（per IP，
      `express-rate-limit`，draft-7 标准 headers）。回归测试在
      `server/tests/routes/rateLimit.test.ts`。
- [x] **复查 Supabase RLS** —— ✅ 已锁（migration 0001+0002）+ 回归测试
      `server/tests/routes/rlsAnon.test.ts`：anon SELECT 只见
      is_participating=true、INSERT/UPDATE/DELETE 静默 drop。
- [ ] **auth 策略重评** —— §1 说过观众自助未在 v1 考虑。决定是开放
      只读还是加验证码 / 免登录 guest 路径。
- [ ] **成本上限**（F10，dashboard 操作，每个 provider 跑一次）——
      Summit 4 站合计预估总用量 < $20，但被刷的话能爆 $1000+/天。
      建议 cap 都设到 **$50/月**（demo 量 10x buffer）。
      - **Gemini** （Google AI Studio）：
        https://aistudio.google.com/app/billing → 选项目 → "Set budget alert" $50；
        硬 cap 走 GCP Console → Billing → Budgets & alerts → 100% 阈值发邮件
        （Google 不支持自动 hard-stop，只能告警；超额需手动停 key）
      - **OpenAI** （Whisper）：https://platform.openai.com/account/limits →
        Monthly budget $50 + Email threshold $25（自动 cap，超了 API 直接 429）
      - **Moonshot Kimi**：https://platform.moonshot.cn/console/account →
        充值上限 + 用量告警（Moonshot 默认预付费，充多少花多少，相对天然 cap）
      - **DeepSeek**：https://platform.deepseek.com/usage →
        同 Moonshot，预付费模式，控制方式 = 充值金额本身
      - 全部 cap 设好后 → 在飞书任务里建一个"每月 1 号查 4 家用量"的提醒，
        Summit 4 站打完后视情况调整或停 key。
- [x] **eval baseline 刷一遍** —— ✅ 改走 prod /api/generate（`scripts/
      eval-via-prod.mjs`），CN 开发机可直跑：
      ```bash
      node scripts/eval-via-prod.mjs        # 12 case ×~7s = ~85s
      ```
      绕开了 Gemini 对 CN billing 账号的位置限制（直跑 server/evals/run-eval.ts
      会 400 "User location not supported"）。结果存 `server/evals/eval-result-prod.json`
      （gitignore 不进 repo）。**注意**：测的是 user-facing chain
      （Gemini→Kimi→DeepSeek），不是 Gemini 单家。2026-05 baseline：
      12/12 pass，全走 deepseek（Gemini 在 Vercel iad1 也 503），avg
      30s p95 52s。Summit 前 1 周再跑一次对账。
- [x] **Vercel preview password protect** —— ✅ 已开（确认于 2026-05）。
      `summit-*.vercel.app` 直链返回 401 + Vercel SSO challenge；
      只有 `summit.aiverygen.ai`（prod alias）公网可达。Settings →
      Deployment Protection → "Vercel Authentication" 已勾。换 prod
      alias 时不动，preview 自动继承。
- [ ] **a11y 色彩对比 follow-up**（F11 残留）—— `--accent` 在白底上
      对比 < 4.5:1（Google #3b82f6=3.67、AWS #ff9900=1.99）。改 brand
      色是设计决策，待你拍板：要么引入 `--accent-on-white` 暗色变体专门
      给小字用、要么放着接受。axe 回归测试 `client/e2e/a11y.spec.ts`
      已暂时 disable 这条规则但保留其它检查。
