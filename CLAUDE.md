...
- [x] **eval baseline 刷一遍** —— ⚠️ 国内开发机直跑会被 Gemini 区域阻止
      （`User location is not supported`，CLAUDE.md §4.4）。从墙外环境
      （Vercel preview deploy / 香港 / 海外 VPN）跑：
      `GEMINI_API_KEY=... npm --workspace server run eval`，
      存 `eval-result.json`。Summit 前 1 周从 prod 环境跑一次留底。
