...
generateRouter.post('/', async (req, res, next) => {
  ...
  try {
    ...
    const response: GenerateResponse = {
      ...
      rationale,
      // Surface the actually-used (provider, model) so the UI can label each
      // result set with "Generated by gemini-2.5-pro · 1.2s". Removes the
      // "did the chain silently fall through?" guesswork without forcing
      // users to read Vercel Logs.
      provider: winning?.providerId as GenerateResponse['provider'],
      model: winning?.model,
      latencyMs,
    };
